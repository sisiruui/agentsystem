...
var v = new Vue({
    ...
    methods: {
        ...
        updateSystemConfig: function () {  // 执行修改
            ...
            if (this.selectSystemConfig.configTypeName == "") {
                humane.error("类型名称不可以为空。");
                return false;
            } else if (this.selectSystemConfig.configValue == "" || parseInt(this.selectSystemConfig.configValue) < 1 ) {
                humane.error("配置数值不可以为空且不可以小于1。");
                return false;
            } else if (this.selectSystemConfig.configTypeValue == "" || parseInt(this.selectSystemConfig.configValue) < 1) {
                humane.error("实际数值不可以为空且不可以小于1。");
                return false;
            }
            var self = this;
            ...
            axios({
                ...
                data: {
                    ...
                    "configTypeValue": self.selectSystemConfig.configTypeValue,
                    "configValue": self.selectSystemConfig.configValue,
                ...
                .then(function (response) {
                    ...
                    if (data < 1) { // 删除失败
                        ...
                        self.$set(self.items[self.indexflag], "configTypeValue", self.selectSystemConfig.configTypeValue);
                        self.$set(self.items[self.indexflag], "configValue", self.selectSystemConfig.configValue);
                        // self.$forceUpdate();
                        humane.success("操作成功！！！");
                    }
        ...
        addSystemConfig: function () { // 这是添加配置的方法
            ...
            if (this.SystemConfig.configTypeName == "") {
                humane.error("类型名称不可以为空。");
                return false;
            } else if (this.SystemConfig.configValue == "" || parseInt(this.SystemConfig.configValue) < 1 ) {
                humane.error("配置数值不可以为空且不可以小于1。");
                return false;
            } else if (this.SystemConfig.configTypeValue == "" || parseInt(this.SystemConfig.configValue) < 1) {
                humane.error("实际数值不可以为空且不可以小于1。");
                return false;
            }
                ...
                axios.post('/systemconfig/financial', {
                    ...
                    configTypeValue: this.SystemConfig.configTypeValue,
                    isStart: this.SystemConfig.isStart,
                    configValue: this.SystemConfig.configValue
                }).then(function (response) {
                    console.log(response);
